...
def solve(segments):
    ...
    if d > segment_length * segment_amount:
        for i in range(len(segments) - 1):
            ...
            l = segment_length / r
            segments[i + 1].position = segments[i + 1].position.lerp((1 - l) * segments[i].position + l * goal_pos,
                                                                     0.99)
    else:
...
while running:
    ...
    # Draw segments
    for i in range(len(segments) - 1):
        pygame.draw.line(screen, rainbow_rgb(i, segment_amount, hue_offset), segments[i].position,
                         segments[i + 1].position, round((segment_amount - i) / (segment_amount / 20)) + 20)
        pygame.draw.circle(screen, rainbow_rgb(i, segment_amount, hue_offset), segments[i].position,
                           round((segment_amount - i) / (segment_amount / 20)) + 10)
        pygame.draw.circle(screen, rainbow_rgb(i, segment_amount, hue_offset), segments[i + 1].position,
                           round((segment_amount - i) / (segment_amount / 20)) + 10)
